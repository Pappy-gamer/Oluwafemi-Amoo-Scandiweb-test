...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    addToCart(state, action) {
      ...
      if (itemIndex >= 0) {
         alert(state.cartItems[itemIndex].Name + ` quantity has been increased`);
        state.cartItems[itemIndex].cartQuantity += 1;
      } else {
        const tempProduct = { ...action.payload, cartQuantity: 1 };
        state.cartItems.push(tempProduct);
        alert(tempProduct.Name + ` added to cart`);
...
export const { addToCart, decreaseCart, increaseCart, getTotals, clearBag, setCurrency } =
  cartSlice.actions;
